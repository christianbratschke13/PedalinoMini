...
\______   \ ____   __| _/____  |  | |__| ____   ____   /     \ |__| ____ |__| /
/ \__    ___/     \   \  \
 |     ___// __ \ / __ |\__  \ |  | |  |/    \ /  _ \ /  \ /  \|  |/    \|  |  /
/    |    | /  \ /  \   \  \ |    |   \  ___// /_/ | / __ \|  |_|  |   |  (  <_>
)    Y    \  |   |  \  | (  (     |    |/    Y    \   )  )
 |____|    \___  >____ |(____  /____/__|___|  /\____/\____|__  /__|___|  /__|
\  \    |____|\____|__  /  /  /
               \/     \/     \/             \/               \/        \/ \__\
\/  /__/ (c) 2018-2024 alf45star https://github.com/alf45tar/PedalinoMini
 */
...
bool rtpInitiator = false; // if device should be RTP Initiator

IPAddress rtpRemoteIp = IPADDR_NONE;
String rtpRemoteHost = "";
unsigned int rtpRemotePort = 5004; // remote port of the RTP device

void printMIDI(const char *interface, midi::StatusByte status,
               const byte *data) {
  midi::MidiType type;
  ...
  case midi::NoteOff:
    ...
    velocity = data[1];
    DPRINTLN(
        "Received from %s  NoteOff 0x%02X   Velocity 0x%02X   Channel %02d",
        interface, note, velocity, channel);
    break;
  ...
  case midi::NoteOn:
    ...
    velocity = data[1];
    DPRINTLN(
        "Received from %s  NoteOn  0x%02X   Velocity 0x%02X   Channel %02d",
        interface, note, velocity, channel);
    break;
  ...
  case midi::AfterTouchPoly:
    note = data[0];
    pressure = data[1];
    DPRINTLN("Received from %s  AfterTouchPoly   Note 0x%02X   Pressure 0x%02X "
             "  Channel %02d",
             interface, note, pressure, channel);
    break;
  ...
  case midi::ControlChange:
    ...
    value = data[1];
    DPRINTLN(
        "Received from %s  ControlChange 0x%02X   Value 0x%02X   Channel %02d",
        interface, number, value, channel);
    break;

  case midi::ProgramChange:
    number = data[0];
    DPRINTLN("Received from %s  ProgramChange 0x%02X   Channel %02d", interface,
             number, channel);
    break;
  ...
  case midi::AfterTouchChannel:
    pressure = data[0];
    DPRINTLN(
        "Received from %s  AfterTouchChannel   Pressure 0x%02X   Channel %02d",
        interface, pressure, channel);
    break;

  case midi::PitchBend:
    bend = data[1] << 7 | data[0];
    DPRINTLN("Received from %s  PitchBend   Bend 0x%02X   Channel %02d",
             interface, bend, channel);
    break;
...
void printMIDI(const char *interface, const midi::MidiType type,
               const midi::Channel channel, const byte data1,
               const byte data2) {
...
void OnAppleMidiNoteOn(byte channel, byte note, byte velocity) {
  ...
  if (interfaces[PED_USBMIDI].midiOut)
    USB_MIDI.sendNoteOn(note, velocity, channel);
  if (interfaces[PED_DINMIDI].midiOut)
  ...
  if (IS_SHOW_ENABLED(interfaces[PED_RTPMIDI].midiIn))
...
void apple_midi_start() {
  ...
  // Connect the handle function called upon reception of a MIDI message from
  // AppleMIDI interface
  ...
  RTP_MIDI.setHandleTimeCodeQuarterFrame(
      OnAppleMidiReceiveTimeCodeQuarterFrame);
...
void OnIpMidiNoteOn(byte channel, byte note, byte velocity) {
  if (!interfaces[PED_IPMIDI].midiIn)
    return;
  ...
  OSCSendNoteOn(note, velocity, channel);
  leds_update(midi::NoteOn, channel, note, velocity);
  if (IS_SHOW_ENABLED(interfaces[PED_IPMIDI].midiIn))
    screen_info(midi::NoteOn, note, velocity, channel);
}
...
void OnOscNoteOn(OSCMessage &msg) {
  if (interfaces[PED_DINMIDI].midiOut)
    DIN_MIDI.sendNoteOn(msg.getInt(1), msg.getInt(2), msg.getInt(0));
}

void OnOscNoteOff(OSCMessage &msg) {
  if (interfaces[PED_DINMIDI].midiOut)
    DIN_MIDI.sendNoteOff(msg.getInt(1), msg.getInt(2), msg.getInt(0));
}

void OnOscControlChange(OSCMessage &msg) {
  if (interfaces[PED_DINMIDI].midiOut)
    DIN_MIDI.sendControlChange(msg.getInt(1), msg.getInt(2), msg.getInt(0));
}
...
void OscSendPedal() {
  ...
  udpOut.sendTo(udpMsg3, oscControllerIP, OSC_CONTROLLER_PORT);
  oscMsg4
      .add((pedals[currentPedal].pressMode & PED_PRESS_1) == PED_PRESS_1 ? 1
                                                                         : 0)
      .send(udpMsg4)
      .empty();
  udpOut.sendTo(udpMsg4, oscControllerIP, OSC_CONTROLLER_PORT);
  oscMsg5
      .add((pedals[currentPedal].pressMode & PED_PRESS_2) == PED_PRESS_2 ? 1
                                                                         : 0)
      .send(udpMsg5)
      .empty();
  udpOut.sendTo(udpMsg5, oscControllerIP, OSC_CONTROLLER_PORT);
  oscMsg6
      .add((pedals[currentPedal].pressMode & PED_PRESS_L) == PED_PRESS_L ? 1
                                                                         : 0)
      .send(udpMsg6)
      .empty();
  ...
  oscMsg9.add((float)pedals[currentPedal].expZero / (ADC_RESOLUTION - 1))
      .send(udpMsg9)
      .empty();
  udpOut.sendTo(udpMsg9, oscControllerIP, OSC_CONTROLLER_PORT);
  oscMsg10.add((float)pedals[currentPedal].expMax / (ADC_RESOLUTION - 1))
      .send(udpMsg10)
      .empty();
...
void OnOscPedalName(OSCMessage &msg) {
  if (msg.getString(0, banks[currentBank][currentPedal].pedalName,
                    MAXACTIONNAME) > 0) {
    DPRINT("OSC message /pedal_name %s received from %s\n",
           banks[currentBank][currentPedal].pedalName,
...
void OnOscPedalAnalogMin(OSCMessage &msg) {
  DPRINT("OSC message /analog_min %f received from %s\n", msg.getFloat(0),
         oscControllerIP.toString().c_str());
  pedals[currentPedal].expZero =
      constrain(msg.getFloat(0) * (ADC_RESOLUTION - 1), 0, ADC_RESOLUTION - 1);
    ...
    while (act != nullptr) {
      if (act->midiMessage == PED_OSC_MESSAGE) {
        if (oscMsg.dispatch(act->oscAddress, OnOscAction)) {
          int value = 255;
          if (oscMsg.isInt(0))
            value = map2(oscMsg.getInt(0), act->midiValue1, act->midiValue2, 0,
                         255);
          else if (oscMsg.isFloat(0))
            value = map2(oscMsg.getFloat(0) * 1024, act->midiValue1 * 1024,
                         act->midiValue2 * 1024, 0, 255);
          fastleds[led_control(act->control, act->led)] = act->color0;
          fastleds[led_control(act->control, act->led)] =
              fastleds[led_control(act->control, act->led)].lerp8(act->color1,
                                                                  value);
          fastleds[led_control(act->control, act->led)].nscale8(
              ledsOffBrightness +
              (ledsOnBrightness - ledsOffBrightness) * value / 255);
          fastleds[led_control(act->control, act->led)] = swap_rgb_order(
              fastleds[led_control(act->control, act->led)], rgbOrder);
          FastLED.show();
          lastLedColor[currentBank][led_control(act->control, act->led)] =
              fastleds[led_control(act->control, act->led)];
        }
      }
      act = act->next;
